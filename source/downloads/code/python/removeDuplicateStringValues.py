...
# Usage:python removeDuplicateStringValues.py android_project_root_dir
# 如果values-fr-rCA存在于values-fr相同的字符串，去除values-fr-rCA中的重复字符串，保留values-fr。这样可以保证在values-fr-rCA下也可以正确读取到资源。如果values-fr存在与values相同的字符串。如去除values-fr中得重复字符串，保留values的条目。
